        ...
        for trend in trend_options:
            ...
            try:
                ...
                for i in range(len(test_s)):
                    ...
                    # roll forward with ACTUAL value (backtesting)
                    history_y = pd.concat([
                        history_y,
                        pd.Series([test_s.iloc[i]], index=[test_s.index[i]])
                    ])
...
top, best = univariate_arima_grid_search(
    series      = train_series,           # your existing pd.Series
    train_range = (FIRST_TRAIN_DATE, LAST_TRAIN_DATE),
